...
class Category_IndexController extends Cl_Controller_Action_NodeIndex 
{
    ...
    public function newAction()
    {
    	...
    	if(has_role('sudo')){
    		...
    		//Get new category
    		$list = Dao_Node_Category::getInstance()->getCategoryByType('new', 3, $row['ts']);
    		$this->setViewParam('newCategorys', $list);
    			
    		//Get popular category
    		$list = Dao_Node_Category::getInstance()->getCategoryByType('hot', 1, $row['ts']);
    		$this->setViewParam('hotCategorys', $list);
    	}
	    ...
	    if(isset($this->ajaxData)) {
            ...
            if (isset($this->ajaxData['result'])) //success
            {
                ...
                else 
                {
                	...
                	$url = node_link('category' , $this->ajaxData['result'], 'upload');
        ...
        Bootstrap::$pageTitle = 'Thêm category mới';
    }
    ...
    public function updateAction()
    {
        ...
        Bootstrap::$pageTitle = 'Cập nhật category';
    }
    ...
    public function searchAction()
    {
        ...
        Bootstrap::$pageTitle = 'Quản lý category';        
    }
    
    public function searchCategorysAction()
    {
    	die('hho');
    	...
    	$dao = Dao_Node_Category::getInstance();
    	...
    	$this->setViewParam('list', $categorys);
    	...
    	//Get new category
    	$list = Dao_Node_Category::getInstance()->getCategoryByType('new', 2);
    	$this->setViewParam('newCategorys', $list);
    	
    	//Get popular category
    	$list = Dao_Node_Category::getInstance()->getCategoryByType('hot', 1);
    	$this->setViewParam('hotCategorys', $list);
    	
    	Bootstrap::$pageTitle = 'Tìm kiếm cover category - ' . $name;
    }
    ...
    public function viewAction()
    {
        ...
        if ($row = $this->getViewParam('row')){
	        //Get list related category 5.9.2013
	        $list = array();
	        $tags = array();
	        ...
	        if(isset($row['tags'])){
	        	...
	        	$list = Dao_Node_Category::getInstance()->find($cond);
	        	if($list){
        	...
        	//TODO: if wasn't original category => get original category, have to upload by admin 
        	$ori_viddeo = array();
        	...
        	$this->setViewParam('ori_category', $ori_viddeo);
        	
        	//Get new category
			$list = Dao_Node_Category::getInstance()->getCategoryByType('new', 15, $row['ts']);
			$this->setViewParam('newCategorys', $list);
        	...
        	Dao_Node_Category::getInstance()->update($whereUpdate, $update);
        }else 
        	Bootstrap::$pageTitle = 'Chi tiết category';
    }
    ...
    {
    	Dao_Node_Category::getInstance()->updateView();
    	die('oki');
    }
    
    public function updateDurationAction()
    {
    	Dao_Node_Category::getInstance()->updateDuration();
    	die('oki');
    }
    ...
    public function tagAction()
    {
    	...
    	if($t['success']){
    		...
    		//Get new category
    		$list = Dao_Node_Category::getInstance()->getCategoryByType('new', 3);
			$this->setViewParam('newCategorys', $list);
			
			//Get popular category
			$list = Dao_Node_Category::getInstance()->getCategoryByType('hot', 2);
			$this->setViewParam('hotCategorys', $list);
    ...
    public function addPlaylistAction(){
    	...
    	$r = Dao_Node_Category::getInstance()->addPlaylist($id);
    ...
    public function manageCategoryAction()
    {
    	...
    	$r = Dao_Node_Category::getInstance()->getCategorysByUser($lu, $page);
    ...
    public function newFbLikeAction()
    {
    	...
    	if(is_rest() && $rt==1)
    	{
    		...
    		if($r['count']>0)
    		{
    			...
    			$dao_category=Dao_Node_Category::getInstance();
    			$rs=$dao_category->findOne(array('id'=>$id));
    			$iid=$rs['result']['iid'];
    		}
    	}
    	$dao_category=Dao_Node_Category::getInstance();
    	$dao_category->fb_like_inc_point($iid,$rt);
    	$dao_category->deleteStaticCacheOfType('vote',1,10);
    	$dao_category->deleteStaticCacheOfType('new',1,10);
    	$r = array('success' => true, 'result' => 'done!');
    	if(is_rest())
    	{
    		send_json($r);
