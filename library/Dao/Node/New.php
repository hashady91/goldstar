<?php
class Dao_Node_New extends Dao_Node_Site
{
    public $nodeType = 'new';
	...
	public function beforeInsertNode($data)
	{
		...
		if (isset($data['tags']) && count($data['tags']) > 0)
		{
			...
			foreach ($data['tags'] as $k => $tag){
				if($tag['name'] == featured_tag() && !has_perm('admin_new')){
					unset($data['tags'][$k]);
				}
			}
	...
	public function afterUpdateComment($where, $data, $row)
	{
           ...
           )
        {
            ...
            Site_Codenamex_Dao_Comment_New::getInstance()->update($cWhere, $dataUpdate);
	 ...
	 */
	public function prepareFormDataForDaoInsert($dataArray = array(), $formName = "New_Form_New")
	{
		return $dataArray;
	}	
	...
	public function afterInsertRelation($data, $newRelations, $currentRow)
	{
		...
		*/
		if($data['o']['status'] == 'approved'){
			//Remove cache home page hot new.local/hot
	...
	public function getNewsByUser($user, $page){
	...
	public function addPlaylist($id){
		...
		if($r['success'] && $r['count'] > 0){
			...
			if($boolen){
				...
				$r = array('success' => false, 'err' => 'New đã tồn tại trong danh sách ưa thích!');
			}
		}else{
			$r = array('success' => false, 'err' => 'New không tồn tại!');
		}
	...
	public function fb_like_inc_point($iid,$rt)
	{
		...
		if ($r['success'] && $r['count'] > 0)
		{
			...
			if ($rt == 1 )//vote up
			{
				parent::updateUserKarmaAndNodePoint($lu,'new_voted_up',$r['result'],$r['result']['u']);
				//if voted up => increase 1 more vote
				$newUpdate = array('$inc' => array('counter.l' => 1));
				$r2 = $this->update($where, $newUpdate);
			} else if ($rt == 4 )//vote down
		    {
	            parent::updateUserKarmaAndNodePoint($lu, 'new_voted_down', $r['result'],$r['result']['u']);
		        $newUpdate = array('$inc' => array('counter.l' => -1));
		        $r2 = $this->update($where, $newUpdate);
		    }
		}
	...
	public function deleteStaticCache($row)
	{
		...
		//Delete cache if exists
		$dir = get_cache_dir();
		$nodelinks = node_link_cache('new', $row);
	...
	public function refreshHotnessAll($data,$typeRedisHotIids)
	{
		...
		if ($r['success'])
		{
			foreach ($r['result'] as $new)
			{
				$redis->zAdd($typeRedisHotIids, $new['counter']['hn'], $new['iid']);
			}
		}
	 ...
	 *
	 * @param unknown_type $data
	 * @param unknown_type $iid  // iid of new relation
	...
	public function incActivityCountAndCheckHotnessCache($data,$iid,$hotNess)
	{
		...
		//calculate hotness 100 new in array redis
		...
		if($result){
			...
			// Removed cahce widget new hot if pagechange + 1 = 1
			if($pageChange == 0){
		...
		// Removed cahce widget new hot if situation + 1 = 1
		if($situation == 0){
