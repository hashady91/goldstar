<?php
class Dao_Node_Category extends Dao_Node_Site
{
	...
	public function beforeInsertNode($data)
	{
		...
		//Get duration
		$url = "http://gdata.youtube.com/feeds/api/categorys/". $data['ytid'];
		$doc = new DOMDocument;
		$doc->load($url);
		...
		//Get views
		$JSON = file_get_contents("https://gdata.youtube.com/feeds/api/categorys/{$data['ytid']}?v=2&alt=json");
		...
		if (isset($data['tags']) && count($data['tags']) > 0)
		{
			...
			foreach ($data['tags'] as $k => $tag){
				if($tag['name'] == featured_tag() && !has_perm('admin_category')){
					unset($data['tags'][$k]);
				}
			}
	...
	public function afterInsertRelation($data, $newRelations, $currentRow)
	{
		...
		*/
		if($data['o']['status'] == 'approved'){
			//Remove cache home page hot category.local/hot
	...
	public function updateView()
	{
		...
		$cond['dao_class'] = 'Dao_Category';
		$this->doBatchJobs($cond, 0, array('updateViewForOneCategory'));
	}
	
	public function updateViewForOneCategory($category){
		$checkYtid = true;
		if(!isset($category['ytid']) || $category['ytid'] == ''){
			parse_str( parse_url( $category['url'], PHP_URL_QUERY ), $my_array_of_vars );
			$category['ytid'] = $my_array_of_vars['v'];
			$checkYtid = false;
		}
		 
		//Get views
		$JSON = file_get_contents("https://gdata.youtube.com/feeds/api/categorys/{$category['ytid']}?v=2&alt=json");
		$JSON_Data = json_decode($JSON);
		$views = $JSON_Data->{'entry'}->{'yt$statistics'}->{'viewCount'};
	...
	public function updateDuration()
	{
		//$cond = array('where' => array());
		$cond['dao_class'] = 'Dao_Category';
		$this->doBatchJobs($cond, 0, array('updateDurationForOneCategory'));
	}
	
	public function updateDurationForOneCategory($category){
		$checkYtid = true;
		if(!isset($category['ytid']) || $category['ytid'] == ''){
			parse_str( parse_url( $category['url'], PHP_URL_QUERY ), $my_array_of_vars );
			$category['ytid'] = $my_array_of_vars['v'];
		...
		//Get duration
		$url = "http://gdata.youtube.com/feeds/api/categorys/". $category['ytid'];
		$doc = new DOMDocument;
		$doc->load($url);
	...
	public function getCategoryByType($type, $limit, $ts){
		$list = array();
		if($type == 'new'){
			if(isset($ts) && $ts > 0){
				$order = array('ts'=>1);
	...
	public function getCategoryList($page, $filter){
		$redis = init_redis(RDB_CACHE_DB);
		$where = array();
		if($filter == 'domestic'){
	...
	public function getCategorysByUser($user, $page){
	...
	public function addPlaylist($id){
		...
		if($r['success'] && $r['count'] > 0){
			...
			if($boolen){
				...
				$r = array('success' => false, 'err' => 'Category đã tồn tại trong danh sách ưa thích!');
			}
		}else{
			$r = array('success' => false, 'err' => 'Category không tồn tại!');
		}
	...
	public function fb_like_inc_point($iid,$rt)
	{
		...
		if ($r['success'] && $r['count'] > 0)
		{
			...
			if ($rt == 1 )//vote up
			{
				parent::updateUserKarmaAndNodePoint($lu,'category_voted_up',$r['result'],$r['result']['u']);
				//if voted up => increase 1 more vote
				$categoryUpdate = array('$inc' => array('counter.l' => 1));
				$r2 = $this->update($where, $categoryUpdate);
			} else if ($rt == 4 )//vote down
		    {
	            parent::updateUserKarmaAndNodePoint($lu, 'category_voted_down', $r['result'],$r['result']['u']);
		        $categoryUpdate = array('$inc' => array('counter.l' => -1));
		        $r2 = $this->update($where, $categoryUpdate);
		    }
		}
	...
	public function deleteStaticCache($row)
	{
		...
		//Delete cache if exists
		$dir = get_cache_dir();
		$nodelinks = node_link_cache('category', $row);
	...
	public function refreshHotnessAll($data,$typeRedisHotIids)
	{
		...
		if ($r['success'])
		{
			foreach ($r['result'] as $category)
			{
				$redis->zAdd($typeRedisHotIids, $category['counter']['hn'], $category['iid']);
			}
		}
	 ...
	 *
	 * @param unknown_type $data
	 * @param unknown_type $iid  // iid of category relation
	...
	public function incActivityCountAndCheckHotnessCache($data,$iid,$hotNess)
	{
		...
		//calculate hotness 100 category in array redis
		...
		if($result){
			...
			// Removed cahce widget category hot if pagechange + 1 = 1
			if($pageChange == 0){
		...
		// Removed cahce widget category hot if situation + 1 = 1
		if($situation == 0){
